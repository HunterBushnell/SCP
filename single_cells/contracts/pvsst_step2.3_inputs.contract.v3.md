...
- **2.1 / 2.2 (cell + geometry)**  
  ...
  - `cell` – an Allen cell (e.g. SST2) built with morphology and mechanisms.
...
- **2.3 (this module)**  
  Consumes:
  - A JSON synapse configuration (`syn_config.json`).
...
### 1.1 `syn_config.json` (input file)
...
1. **Config normalization (2.3.1–2.3.2)**  
   - Load `syn_config.json`, split into `sim_raw` and `groups_raw`.  
...
- Loads and parses `syn_config.json`.
...
syn_cfg_path = TUNE_DIR / "syn_config.json"
