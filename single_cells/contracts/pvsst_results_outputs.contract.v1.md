...
* **Output locations (defaults)**
  * **Run folders:** `{tune_dir}/output_data/<output_stem>/`
    * `tune_dir` is the cell tune folder, e.g. `PV-SST/single_cells/cells/SST2/tunes/seg_tuned`.
  ...
  * **SLURM logs (per run):** `{tune_dir}/output_data/<output_stem>/logs/`
    ...
    * Rotated logs may still appear under `PV-SST/single_cells/logs/old/<YYYYMMDD_HHMMSS>/`.
...
results = run_sim.load_results("cells/SST2/tunes/seg_tuned/output_data/slurm_20251218_231500")
plotting.plot_results(results)
```
  * Notebook alternative: `PV-SST/single_cells/5_analysis.ipynb`
