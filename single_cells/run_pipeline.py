...
Minimal usage (from repo root or tune dir):
    python -m run_pipeline --tune-dir cells/SST2/tunes/seg_tuned --mode multi --n-trials 10

Defaults:
    - Uses sim_config.json and syn_config.json in --tune-dir.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cell", type=str, default=None,
                   help="Cell name (e.g., SST2, PV). Used if --tune-dir is not provided.")
...
def main() -> None:
    ...
    # Paths
    sim_path = tune_dir / "sim_config.json"
    syn_path = tune_dir / "syn_config.json"
    manifest_path = tune_dir / "manifest.json"

    if not sim_path.is_file():
        raise FileNotFoundError(f"Missing sim_config.json in {tune_dir}")
    if not syn_path.is_file():
        raise FileNotFoundError(f"Missing syn_config.json in {tune_dir}")
    ...
    cell_cfg.setdefault("cell_name", tune_dir.parent.name)  # e.g., SST2 or PV
