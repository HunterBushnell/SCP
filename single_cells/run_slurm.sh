...
CELL=${CELL:-SST2}
TUNE=${TUNE:-seg_tuned}
TUNE_DIR=${TUNE_DIR:-/home/hrbncv/PV-SST/single_cells/cells/${CELL}/tunes/${TUNE}}
...
SAVE_ENABLED=0
SAVE_STEM=""
if [[ -f "${TUNE_DIR}/sim_config.json" ]]; then
    read -r SAVE_ENABLED SAVE_STEM < <(TUNE_DIR="$TUNE_DIR" python - <<'PY'
import json
import os
from pathlib import Path
cfg_path = Path(os.environ.get("TUNE_DIR", "")) / "sim_config.json"
...
CMD=(python /home/hrbncv/PV-SST/single_cells/run_pipeline.py
    --tune-dir "$TUNE_DIR"
...
if [[ -n "${SLURM_ARRAY_TASK_ID:-}" && "${SLURM_ARRAY_TASK_ID}" == "0" && "${MERGE_ARRAY}" == "1" ]]; then
    ...
    MERGE_CMD=(python /home/hrbncv/PV-SST/single_cells/scripts/merge_array_results.py
        --input-dir "$PARTS_DIR"
        --output-dir "$RUN_ROOT"
        --job-id "$ARRAY_JOB_ID"
