...
Usage (from repo root):
  python single_cells/scripts/sample_inputs.py \
    --tune single_cells/cells/SST2/tunes/seg_tuned \
    --group pn_exc \
    --runs 20 \
    --bin-ms 5 \
...
# Ensure repo root and single_cells/ are on sys.path so modules_local can be imported when run as a script
REPO_ROOT = Path(__file__).resolve().parents[2]
SINGLE_CELLS_ROOT = REPO_ROOT / "single_cells"
for p in (REPO_ROOT, SINGLE_CELLS_ROOT):
...
def main():
    ...
    ap.add_argument("--tune", required=True, help="Path to tune directory containing sim_config.json and syn_config.json")
