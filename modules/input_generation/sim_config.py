...
def _normalize_sim_config(sim_cfg_raw: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Ensures keys:
      ...
      - save_output_metrics (optional: calculate/save Step 6 metrics after saving a run)
      - save_output_metrics_preset (optional path to output-metrics preset JSON)
      - save_output_metrics_formats (optional list containing json/csv/md)
    ...
    sim_cfg["save_output_metrics"] = bool(sim_cfg.get("save_output_metrics", False))
    sim_cfg["save_output_metrics_overwrite"] = bool(
        sim_cfg.get("save_output_metrics_overwrite", True)
    )
    metrics_preset = sim_cfg.get("save_output_metrics_preset")
    if metrics_preset not in (None, "", False):
        sim_cfg["save_output_metrics_preset"] = str(metrics_preset)
    else:
        sim_cfg["save_output_metrics_preset"] = None
    metric_formats = sim_cfg.get("save_output_metrics_formats", ["json", "csv", "md"])
    if isinstance(metric_formats, str):
        metric_formats = [part.strip() for part in metric_formats.split(",") if part.strip()]
    if not isinstance(metric_formats, (list, tuple)):
        raise ValueError("sim['save_output_metrics_formats'] must be a list or comma-separated string")
    metric_formats = [str(fmt).strip().lower().lstrip(".") for fmt in metric_formats]
    invalid_metric_formats = [fmt for fmt in metric_formats if fmt not in {"json", "csv", "md"}]
    if invalid_metric_formats:
        raise ValueError(
            "sim['save_output_metrics_formats'] entries must be json, csv, or md "
            f"(got {invalid_metric_formats!r})"
        )
    sim_cfg["save_output_metrics_formats"] = metric_formats
