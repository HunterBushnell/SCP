...
def resolve_export_paths(
    ...
    *,
    ...
    """
    Public wrapper for resolving requested single-plot-panel export paths.
    """
...
def load_single_plot_preset(
    *,
    ...
    path = (
        root / "modules" / "analysis" / "analysis_presets" / "single_plot.json"
...
def plot_single_plot_panel_from_results(
    results: Dict[str, Any],
    *,
    ...
    """
    Build a single-plot stacked panel figure from one run's saved results.
...
def resolve_single_run_for_single_plot_panel(selection: Dict[str, Any]) -> tuple[Path, list[str]]:
    """
    Resolve one run path from selection for single-plot panel usage.
...
def run_single_plot_from_selection(
    ...
    *,
    ...
    """
    Resolve one run from analysis selection and produce a compact single-plot panel.
    ...
    preset_overrides lets notebook/UI controls adjust a small subset of preset values
    without mutating the JSON preset. When allow_export=False, existing exports may
    still be reused, but regenerated figures are not written to disk.
    """
    warnings: list[str] = []
    run_dir, run_warnings = resolve_single_run_for_single_plot_panel(selection)
    ...
    if preset_overrides:
        panel_cfg.update(dict(preset_overrides))
    ...
    from modules import run_sim  # local import to keep module dependencies light

    results = run_sim.load_results(run_dir)
    if not bool(allow_export):
        panel_cfg = dict(panel_cfg)
        panel_cfg["export_path"] = None
        panel_cfg["export_formats"] = []

    panel_result = plot_single_plot_panel_from_results(
        results,
        run_dir=run_dir,
        **panel_cfg,
    )
...
def display_single_plot_result(
    ...
    *,
    ...
    width/height scale saved preview images in notebook output without changing the
    exported publication files.
    ...
    try:
        from IPython.display import HTML, Image, SVG, display  # type: ignore
    except Exception:
        return preview
    ...
    if suffix in {".png", ".jpg", ".jpeg", ".gif", ".webp"}:
        display(Image(filename=str(preview), width=width, height=height))
        return preview
